export class DoubleLinkedList {
  ...
  add(data = "") {
    let listItem = {};
    if (!this.data.length) {
      listItem = { data: data, prev: null, next: null };
    } else {
      listItem = {
        data: data,
        prev: null,
  ...
  addAt(data, index) {
    ...
    this.data.splice(index, 0, {
      data: data,
      next: this.data[index] || null,
      prev: this.data[index - 1] || null,
    });

    if (this.data.length > 1 && index !== this.data.length - 1 && index) {
      this.data[index].prev = this.data[index - 1];
      this.data[index - 1].next = this.data[index];
      this.data[index + 1].prev = this.data[index];
    }
